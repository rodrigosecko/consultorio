...
class Devolucion_model extends CI_Model {
    ...
    function get_data_table() {//obtiene los datos de la tabla tipo_documento en array result
        $query = $this->db->query('SELECT x.asignacion_id,x.activo_id,z.descripcion,y.persona_id,y.nombres,y.paterno,y.materno,x.fecha_asign,x.activo,z.url,z.imagen from asignacion x
            LEFT JOIN
            persona y
    ...
    function get_data_histo() {//obtiene los datos de la tabla tipo_documento en array result
        $query = $this->db->query('SELECT x.devolucion_id,x.activo_id,z.descripcion,y.persona_id,y.nombres,y.paterno,y.materno,x.motivo,x.activo,x.fecha_devolucion from devolucion x
            LEFT JOIN
            persona y
            on y.persona_id=x.empleado_id
            LEFT JOIN
